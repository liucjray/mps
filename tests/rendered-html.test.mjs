...
test("renders the dark circles child knowledge page", async () => {
  ...
  assert.equal(response.headers.get("cache-control"), "public, max-age=300, stale-while-revalidate=86400");
  const html = await response.text();
  assert.match(html, /<h1>黑眼圈怎麼看？[\s\S]*先分辨色澤、血管與立體陰影。[\s\S]*<\/h1>/i);
  assert.match(html, /<title>黑眼圈怎麼看？成因、分類與外觀評估｜新北雙和店｜瑪菲斯皮膚覆蓋專家<\/title>/i);
  assert.match(html, /<meta name="description" content="整理黑眼圈四大類型（色素型、血管型、結構型、混合型）成因與外觀辨析表。介紹眼下皮膚拉平測試、處置途徑三方客觀比對與諮詢前評估重點。"\/>/i);
  ...
  assert.match(html, /黑眼圈四大類型成因與外觀辨析表/);
  assert.match(html, /居家自我檢測：眼下皮膚「輕拉測試」/);
  assert.match(html, /眼周困擾處置途徑三方客觀比對表/);
  assert.match(html, /Allergic Shiners/);
  assert.match(html, /新北市中和區/);
  assert.match(html, /捷運南勢角站/);
  assert.match(html, /鄰近捷運南勢角站，建議出發前透過地圖規劃路線/);
  assert.match(html, /maps\/search\/\?api=1&amp;query=%E6%96%B0%E5%8C%97%E5%B8%82%E4%B8%AD%E5%92%8C%E5%8D%80%E6%99%AF%E6%96%B0%E8%A1%97347%E8%99%9F/);
  assert.match(html, /景新街347號9樓之9（台北富邦銀行樓上）/);
  assert.match(html, /私訊諮詢前可先整理：① 在意部位 ② 出現時間與狀態 ③ 局部的色澤或光影感受/);
  assert.match(html, /11:00–19:00（預約制）/);
