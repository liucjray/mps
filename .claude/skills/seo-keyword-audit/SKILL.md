...
1. **先讀 `docs/seo/keyword-database.md`**——這是持續維護的目標關鍵字狀態表（關鍵字、意圖、對應頁面、GSC 現況、優先度），2026-09-22 建立。這一步不用重新臨場想關鍵字，直接照這張表核對哪些已覆蓋、哪些排名不佳、哪些還沒實測過。表裡的「核心發現」段落記錄了目前已知的策略結論（例如「商業意圖詞 vs 資訊意圖詞」的真實搜尋量落差），先讀過再判斷本次方向，不要重新推導一次已經有結論的東西。
2. 盤點目前站上實際在用的關鍵字／主題：`app/site.ts`、`app/services.ts`、`app/knowledge/*`（若有）、`public/llms.txt`、`public/llms-full.txt`，比照 260917 計畫文件的四層金字塔分類，確認 `keyword-database.md` 有沒有遺漏新上線的頁面/關鍵字，遺漏就補進去。
3. 對照 260917 計畫文件與 vault `04-SEO-AEO` 裡「規劃要做但還沒上線」的關鍵字／內容缺口，判斷哪些已經覆蓋、哪些還沒、有沒有新出現的缺口或新的競品訊號。
4. 這一步只做盤點與比對，不查即時排名（排名查核在第 3 步做），先在腦中／草稿整理出「本次要實測排名的關鍵字清單」，優先挑 `keyword-database.md` 裡優先度高、還沒實測過或數據已經舊的項目。
5. **第 3 步查完 GSC 之後，回頭把 `keyword-database.md` 對應行的「GSC 近 N 天」欄與日期更新**（這是活文件，不是查完就丟）；若查出的結論推翻了「核心發現」段落的既有判斷，也要同步改寫那段，不要留著矛盾的舊結論。
