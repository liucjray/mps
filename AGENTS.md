...
For UI or responsive design changes, use the `frontend-design` skill, then verify with local desktop/mobile screenshots. For a new page or a redesign where visual direction is undecided, also read `design-taste-frontend` first to state a one-line Design Read (page kind, audience, vibe, design-system fit) before writing code — it complements `frontend-design`'s anti-template principles with a concrete brief-to-direction process. For metadata, structured data, sitemap, robots, or AI-search changes, use `seo-aeo-best-practices` and verify rendered HTML with `npm test`. For a periodic or ad hoc keyword-coverage-and-ranking health check (distinct from `seo-aeo-best-practices`, which validates metadata/structured-data mechanics rather than keyword strategy), use the vendored `seo-keyword-audit` skill (`.claude/skills/seo-keyword-audit/SKILL.md`); it logs every run to `docs/seo/keyword-ledger.md` and requires both an agy self-check and an independent `codex` second opinion before implementing or committing any optimization. Read selected skill instructions completely and use only the relevant skill.
...
Never commit `.env.cloudflare`, `.env.gsc`, analytics credentials, or tokens; use documented environment names and GitHub Actions Repository Variables for public build configuration. `.env.gsc` holds the same GSC service-account JSON as the `GSC_CREDENTIALS` GitHub Actions secret, kept locally (see `.env.gsc.example`) so `npm run query:gsc` can pull real Search Console ranking data outside CI, e.g. for the `seo-keyword-audit` skill. `NEXT_PUBLIC_GA_MEASUREMENT_ID` is a public GA4 identifier, not a secret, and must be supplied at build time. A push to `main` triggers the Cloudflare deployment workflow, so verify tests and the diff before pushing.
